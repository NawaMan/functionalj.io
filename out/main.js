...
var author$project$Main$initialModel = {count: 1};
		...
		switch (msg.$) {
			...
			case 'Decrement':
				return _Utils_update(
					model,
					{
						count: (model.count <= 0) ? 0 : (model.count - 1)
					});
			default:
				return _Utils_update(
					model,
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text('Reset to zero')
							]))
					])),
				A2(
				elm$html$Html$button,
